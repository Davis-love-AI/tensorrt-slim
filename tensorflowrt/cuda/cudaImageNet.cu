...
// gpuPreImageNet
__global__ void gpuPreImageNet(float2 scale, float4* input, int iWidth,
...
// gpuPreImageNetMean
__global__ void gpuPreImageNetMean(float2 scale, float4* input, int iWidth,
                                   float* output, int oWidth, int oHeight, float3 mean_value)
{
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    const int n = oWidth * oHeight;
    ...
    const float4 px  = input[ dy * iWidth + dx ];
    const float3 rgb = make_float3(px.x - mean_value.x, px.y - mean_value.y, px.z - mean_value.z);
