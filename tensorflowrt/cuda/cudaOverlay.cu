...
/* ============================================================================
 * Single 2D box CUDA overlay.
 * ========================================================================== */
template<typename T>
__global__ void gpuRectOutlines(T* input, T* output, int width, int height,
                                float4 box2d, float4 color)
{
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    const float alpha = color.w / 255.0f;
    const float ialph = 1.0f - alpha;
    // Convert 2D box to absolute coordinates. TODO: check coordinates...
    float4 r = box2d;
    r.x *= width;
    r.z *= width;
    r.y *= height;
    ...
    // Paint if inside; TODO: fix this shit branching.
    if( fy >= r.y && fy <= r.w /*&& (eq_less(fx, r.x, ep) || eq_less(fx, r.z, ep))*/ )
    {
        if( fx >= r.x && fx <= r.z /*&& (eq_less(fy, r.y, ep) || eq_less(fy, r.w, ep))*/ )
        {
    ...
    uint32_t width, uint32_t height, const float4& box2d, const float4& color)
{
    ...
    gpuRectOutlines<float4><<<gridDim, blockDim>>>(input, output, width, height, box2d, color);
    return cudaGetLastError();
}
